...
static REAL cos1_6=(REAL)cos(PI/6.0*1.0);
static REAL cos2_6=(REAL)cos(PI/6.0*2.0);
...
void Mpegtoraw::layer3initialize(void)
{
  ...
  // Calculate win
  {
    ...
    for(i=0;i<18;i++)
      win[0][i]=win[1][i]=(REAL)(0.5*sin(PI_72*(double)(2*i+1))/cos(PI_72*(double)(2*i+19)));
    for(;i<36;i++)
      win[0][i]=win[3][i]=(REAL)(0.5*sin(PI_72*(double)(2*i+1))/cos(PI_72*(double)(2*i+19)));

    for(i=0;i<6;i++)
    {
      win[1][i+18]=(REAL)(0.5/cos(PI_72*(double)(2*(i+18)+19)));
      win[3][i+12]=(REAL)(0.5/cos(PI_72*(double)(2*(i+12)+19)));
      win[1][i+24]=(REAL)(0.5*sin(PI_24*(double)(2*i+13))/cos(PI_72*(double)(2*(i+24)+19)));
      win[1][i+30]=win[3][i]=(REAL)0.0;
      win[3][i+6 ]=(REAL)(0.5*sin(PI_24*(double)(2*i+1))/cos(PI_72*(double)(2*(i+6)+19)));
    }
    for(i=0;i<12;i++)
      win[2][i]=(REAL)(0.5*sin(PI_24*(double)(2*i+1))/cos(PI_24*(double)(2*i+7)));
  }

  for(i=0;i<9;i++)
	  cos_18[i]=(REAL)(cos(PI_18*double(i)));
  for(i=0;i<9;i++)
	  hsec_36[i]=(REAL)(0.5/cos(PI_36*double(i*2+1)));
  for(i=0;i<3;i++)
	  hsec_12[i]=(REAL)(0.5/cos(PI_12*double(i*2+1)));
  ...
  for(i=0;i<8;i++)
    for(j=0;j<2;j++)
      for(k=0;k<16;k++)POW2_1[i][j][k]=(REAL)(pow(2.0,(-2.0*i)-(0.5*(1.0+j)*k)));
 
  {
    static REAL TAN12[16]=
    { 0.0f,        0.26794919f, 0.57735027f  , 1.0f,
      1.73205081f, 3.73205081f, 9.9999999e10f,-3.73205081f,
      -1.73205081f,-1.01f,      -0.57735027f,  -0.26794919f,
      0.0f,        0.26794919f, 0.57735027f,   1.0f};

    for(i=0;i<16;i++)
    {
      rat_1[i].l=(REAL)(TAN12[i]/(1.0+TAN12[i]));
      rat_1[i].r=(REAL)(1.0/(1.0+TAN12[i]));
    }
  }
...
#define IO1 ((double)0.707106781188)
  ...
  for(i=1;i<64;i++)
    if((i%2)==1)
    {
      rat_2[0][i].l=(REAL)pow(IO0,(i+1)/2);
      rat_2[1][i].l=(REAL)pow(IO1,(i+1)/2);
      rat_2[0][i].r=
      rat_2[1][i].r=1.f;
    }
    else
    {
      rat_2[0][i].l=
      rat_2[1][i].l=1.f;
      rat_2[0][i].r=(REAL)pow(IO0,i/2);
      rat_2[1][i].r=(REAL)pow(IO1,i/2);
    }
